...
6. Candidate calls `GET /api/v1/candidate/jobs/recommended`; jobs already applied by the candidate, expired jobs, deleted jobs, unpublished jobs and jobs from inactive companies are excluded.
7. Recommendations are sorted by `matchScore DESC`, then `createdDate DESC`. MVP score uses skill 40%, title 20%, location/work mode 15%, experience 15% and salary 10%.
